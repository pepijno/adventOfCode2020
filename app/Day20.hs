...
allOrientations x = rots ++ (map transpose rots)
  where rots = [rot90 x, rot90 $ rot90 x, rot90 $ rot90 $ rot90 x, x]

allOrientations' :: [([[a]] -> [[a]])]
allOrientations' = rots ++ (map (\x -> transpose . x) rots)
  where rots = [rot90, rot90 . rot90, rot90 . rot90 . rot90, id]

isNeighbour x y = not $ null $ intersect ys xs
  where xs = map head $ allOrientations x
...
findShortest xs
  ...
  where xs' = filter ((>) 12 . length . snd) $ sortBy (compare `on` length . snd) $ zip [0..] xs
...
solve1 xs = M.foldlWithKey (\p k _ -> k * p) 1 $ M.filter ((==2) . length) $ updateNeighbours (M.keys sqs) $ M.fromList $ findAllNeighbours (M.keys sqs) sqs
  where sqs = M.fromList $ map ((tileRuleToId . head) &&& tail) $ groupPairs xs
...
findMonsters xss = filter (isMonster) pairs
  where pairs = concat $ map (\x -> map (\y -> (x, y)) [1..(l - 2)]) [0..(l - 19)]
        l = length (xss !! 0)
        isMonster (x, y) =
          ((xss !! (y - 1)) !! (x + 18)) == '#' &&
          ((xss !! (y)) !! (x)) == '#' &&
...
solve2 xs = minimum $ map (\x -> e - x * 15) $ map (length . findMonsters) $ allOrientations bla
  where bla = concat $ map (concatLine) $ map (map submatrix) $ map reverse $ transformImage [] $ map (\x -> toImage [] x sqs) $ fst $ last $ take 144 $ iterate (uncurry constructImage) ([[corner]], removeFromMap corner m)
