...
firstBus minutes ids = minimumBy (compare `on` snd) . zip ids $ map (\x -> x - (minutes `mod` x)) ids
...
solve1 :: [String] -> Int
solve1 xs =
  let (bus, m) = firstBus minutes $ parseInput $ last xs
   in bus * m
  where
