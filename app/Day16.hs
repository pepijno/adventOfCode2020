...
parseInput xs = ((map (unsafeParse parseCondition) . head) ps
...
solve1 xs = (sum . map (sumOfNotValid cs)) ts
  where (cs, _, ts) = parseInput xs

allValid :: [Int] -> Condition -> Bool
allValid xs c = all (\x -> isValid x c) xs

repeatFilter :: [[Condition]] -> [(Int, Condition)] -> [(Int, Condition)]
repeatFilter is kps = ((head . map (\(a,b) -> (a, head b)) .
  filter ((==1) .  length . snd) .
    zip [0..] .
      map (filter (not . flip elem vs))) is)
      :kps
  where vs = map snd kps
